...
{ 
  ...
  P1REN = 0x41;  //set p1.6 p1.0 (button) to use pull-down resistor
  P1OUT = 0x00;  //everything in P1 OFF, pull-(up/down) on p1.6 & p1.0 set to down
  ...
  P1IE |= 0x10; //enable interrupt on P1.4
  P1IES|= 0x10; //set interrupt mode on P1.4 to detect Hi/low edge
  P1IFG&=~0x10; //clear interrupt flag on P1.4
...
void readButton(){
  ...
  buttonSamples[0] = P1IN & 0x40;
  for(int i = 1; i < nButtonSamples; i++){
    buttonSamples[i] = P1IN & 0x40;
    if(buttonSamples[i] != buttonSamples[i-1]){
      sameFlag = false;
    }
    delay(deltaT);
  }
  if(sameFlag && (buttonSamples[0] == 0x40)){ //if all samples on
    buttonState = true;
  }
...
__interrupt void Timer_A(void){
  ...
  //TODO: move sample read, display update code here
}
